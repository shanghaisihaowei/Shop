  ...
  <div id="main">
    <div class="top-next cle">
      <div class="fr">
        ...
        <a class="btn" id="checkout-top" @click="balanceCount"
          >&nbsp;去结算&nbsp;</a
        >
      </div>
    </div>
    <div class="cart-box" id="cart-box">
      <div class="hd">
        <span class="no2" id="itemsnum-top"
          >{{ goods.goods_list.length }}件</span
        >
      ...
      <div class="goods-list">
        <ul>
          ...
          >
            ...
            <div class="price-xj">
              <p>
                <em>￥{{ item.goods.shop_price }}元</em>
              </p>
            </div>
            <div class="nums" id="nums" style="margin-left: 10px">
              <span
                style="visibility: visible"
                title="减少1个数量"
                @click="reduceCartNum(index, item.goods.id)"
                >-</span
              >
              <input
                style="width: 40px; border: 1px solid #ddd"
                type="text"
                v-model="item.nums"
              />
              <span
                ...
                class="add"
                title="增加1个数量"
                @click="addCartNum(index, item.goods.id)"
                >+</span
              >
            </div>
            <div class="price-xj">
              <span></span>
              <em id="total_items_3137"
                >￥{{ item.goods.shop_price * item.nums }}元</em
              >
            </div>
            <div class="del">
              <a class="btn-del" @click="deleteGoods(index, item.goods.id)"
                >删除</a
              >
            </div>
          </li>
      ...
      <div class="fd cle">
        ...
        <div style="text-align: right" id="price-total">
          <span id="selectedCount">{{ goodsNmus }}</span>
          件，小计：<span id="totalSkuPrice">￥{{ totalPrice }}元</span>
        </div>
        <div style="text-align: right">运费：￥{{ freight }}元</div>
        <div style="text-align: right">优惠：￥{{ discounts }}元</div>
        <div style="text-align: right; color: #999999">(注：满1000减100)</div>
...
export default {
  data() {
    return {
      ...
      payWrapList: [
        {
          id: 1,
          img: "https://shop.56yhz.com/images%5Calipay.jpg",
        },
      ],
      payWrapActive: null,
      addressActive: null,
  ...
  watch: {
    goods: {
      handler(val) {
        this.goodsNmus = 0;
        val.goods_list.forEach((item) => {
          this.goodsNmus += Number(item.nums);
        });
      },
      immediate: true,
      deep: true,
  ...
  computed: {
    freight() {
      if (this.totalPrice < 100 && this.goods.totalPrice) {
        return 10;
      } else {
        return 0;
      }
  ...
  methods: {
    ...
    setTotalPrice() {
      ...
      for (var i = 0; i < goods_list.length; i++) {
        totalPrice =
          totalPrice + goods_list[i].nums * goods_list[i].goods.shop_price;
      }
      this.totalPrice = totalPrice;
    },
